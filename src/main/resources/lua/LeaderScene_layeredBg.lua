print('Loading LeaderScene_layeredBg.lua from Civ Blitz');
-- The following is a copy of original UI/LeaderScene.lua, with slight custom modifications by SztoDwa, 2023.
-- Copyright Firaxis, 2016-2018
-- ************************************************************************************************************************
--	LeaderScene
...
function GenerateLayers(selectedPlayerID:number)
	...
	if (playerConfig ~= nil) then
		...
		-- OVERRIDE START
		-- For leaders that have a DiplomacyInfo entry but SceneLayers are also set, load multiple background layers.
		-- by SztoDwa, 2023
        ...
        if diplomacyInfo and diplomacyInfo.BackgroundImage then
            if numLayers > 1 then
                for i = 1, numLayers, 1 do
                    local layer:table = CreateBackgroundLayer(diplomacyInfo.BackgroundImage.."_"..i, unloadTextures);
...
print('OK loaded LeaderScene_layeredBg.lua from Civ Blitz')